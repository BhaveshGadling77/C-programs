...
char *grade_range(double marks) {
    if(marks > 88.00 && marks <= 100.00)
        return "AA";
    else if(marks > 77.00 && marks <= 88.00)
        return "AB";
    else if(marks > 60.00 && marks <= 77.00)
        return "BB";
    else if(marks > 59.00 && marks <= 60.00)
        return "BC";
    else if(marks > 54.00 && marks <= 59.00)
        return "CC";
    else if(marks > 51.00 && marks <= 54.00)
        return "CD";
    else if(marks > 36.00 && marks <= 51.00)
        return "DD";
    else if(marks >= 0.00 && marks <= 36.00)
        return "FF";
    else
        return "Invalid";
}
...
float CGPA(sub s[], marks m[], long int mis, int r, int p){
    inssort(m, r);
    ...
    int Denominator = 0;
    for(int i = 0; i < p; i++){
        Numerator += (get_points(grade_range((m[row].score)[i])) * s[i].credits);
        Denominator += s[i].credits;
    }
    return  ((float)Numerator / Denominator);
}
float SGPA(sub s[], marks m[], long int mis, int r, int p, int sem){
    inssort(m, r);
...
void failed_sub(sub s[], marks m[], long int mis, int r, int p){
    inssort(m, r);
    int row = binary_search_mis(m, mis, r);
    if(row == -1){
        printf("not found\n");
    ...
    for(int i = 0; i < p; i++){
        if(get_points(grade_range((m[row].score)[i])) == 0)
            printf("%s\n", s[i].sub_name);
    }
}
        ...
        if(strcmp(inputs[0], "grade") == 0){
            ...
            if(strcmp(inputs[1], "all") == 0){
                for (int i = 0; i < r; i++) {
                    printf("%ld ", m[i].MIS);
                    for (int j = 0; j < p; j++)
                        printf("%s ", grade_range((m[i].score)[j]));
                    printf("\n");
                }
            }
            else if (i >= 3) {
            ...
            printf("%.2f\n", SGPA(s, m, atol(inputs[1]), r, p, atoi(inputs[2])));
        }
            ...
            failed_sub(s, m, atol(inputs[1]), r, p);
        }
